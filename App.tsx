...
export default function App() {
  ...
  }, []); // Only runs on unmount of App, but we handle destroy manually in lobby back
  ...
  const setupConnection = (c: DataConnection) => {
      c.on('data', (data: any) => {
          ...
          if (data.type === 'MOVE') {
              ...
              triggerAIEmote(data.emoteId); // Reuse AI Emote visual for Opponent
          }
      });
      c.on('close', () => {
          setConnStatus('ERROR');
          // alert("Opponent disconnected."); // Removed to avoid blocking UI
          setView('MENU');
      });
  };
      ...
      <div className="w-full max-w-md space-y-6">
          ...
          {connStatus === 'ERROR' && (
             <div className="text-center space-y-4">
                ...
                <p className="text-red-400 tracking-widest uppercase">Connection Lost</p>
                <button onClick={() => setConnStatus('IDLE')} className="text-stone-400 underline text-sm">Return</button>
             </div>
          )}
      </div>
    </div>
